...
df_report, _ = fetch_data_monthly('A6')
    ...
    columnDefs=[
        {
            ...
            "mould_id", "month_total_stop", "month_total_dt", "stop_percentage"
...
app.layout = html.Div([
    ...
    style={
        ...
        "width": "60%",  # Set a smaller width for the grid
        "margin": "20px auto",  # Center the grid horizontally and add vertical spacing
        "textAlign": "center",  # Center the content inside the grid
    })
...
@callback(
    ...
    Output("grid_monthly", "rowData"),  
    Input("monthly_report", 'clickData'),  
)
def update_shift_data(clickData):
    if clickData is None:
        # Return an empty figure and empty data for the grid
        return go.Figure(), []
    ...
    df_overall, df_detailed  = fetch_data_monthly(machine_code)
    print(df_overall)
    print(df_detailed)

    return daily_graph, df_detailed.to_dict("records")
